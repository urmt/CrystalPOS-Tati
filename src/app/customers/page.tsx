...
import { 
  ...
  TableRow, Paper, TextField, InputAdornment, Chip
} from '@mui/material';
import { Search, Download, Refresh } from '@mui/icons-material';
import { supabase } from '@/lib/supabase';
import { Customer } from '@/types';
import { formatCurrency as formatCRC } from '@/utils/format';
...
export default function CustomersPage() {
  ...
  return (
    <DashboardLayout currentPage="customers" title={`Customers (${totalCustomers})`} actions={actions}>
      {/* Stats */}
      <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
        ...
        <Paper sx={{ p: 2, flex: 1 }}>
          ...
          <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#6B4C9A' }}>{formatCRC(totalRevenue)}</Typography>
        </Paper>
      </Box>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={customer.id} hover>
                ...
                <TableCell sx={{ fontWeight: 'bold', color: '#6B4C9A' }}>
                  {formatCRC(customer.total_purchases || 0)}
                </TableCell>
                ...
                <TableCell>
                  {customer.special_requests && (
                    <Chip label={customer.special_requests} size="small" sx={{ bgcolor: '#D4AF37' }} />
                  )}
